...
BUFFER_ZONE_LOW = 240000
BUFFER_ZONE_HIGH = 250000
SUMMARY_TARGET_TOKENS = 200000
...
class HierarchicalMemoryManager:
    ...
    def _calculate_token_count(self, context_data: list) -> int:
        total_tokens = 0
        for item in context_data:
            if item.get("role") == "assistant" and "token_usage" in item:
    ...
    def _load_context(self) -> list:
        ...
        if os.path.exists(context_path):
            ...
            if not data:
                return [{"type": "metadata", "token_count": 0, "in_buffer_zone": False, "daily_summarized": False, "archived_to_raw": False}]
            
            first_item = data[0]
            if isinstance(first_item, dict) and first_item.get("type") == "metadata":
                processed_data = data
            else:
                token_count = self._calculate_token_count(data)
                metadata = {"type": "metadata", "token_count": token_count, "in_buffer_zone": False}
        ...
        return [{"type": "metadata", "token_count": 0, "in_buffer_zone": False}]
    ...
    def add_memory(self, role: str, text: str, model: str = None, token_usage: dict = None, memory_record: dict = None):
        ...
        if memory_record:
            ...
            if role == "assistant" and token_usage and isinstance(token_usage, dict):
                if (len(self.context_buffer) > 0 and 
                    isinstance(self.context_buffer[0], dict) and 
                    self.context_buffer[0].get("type") == "metadata"):
                    self.context_buffer[0]["token_count"] += token_usage.get("total", 0)
                    self._update_buffer_zone_status()
            
            self.save_context()
            
            if (len(self.context_buffer) > 0 and 
                isinstance(self.context_buffer[0], dict) and 
                self.context_buffer[0].get("in_buffer_zone")):
                logger.info("[MemoryDB] 进入缓冲区，触发上下文总结")
    ...
    def _update_buffer_zone_status(self):
        if not self.context_buffer:
            return
        metadata = self.context_buffer[0]
        if not isinstance(metadata, dict) or metadata.get("type") != "metadata":
            return
        
        token_count = metadata.get("token_count", 0)
        in_buffer_zone = BUFFER_ZONE_LOW <= token_count <= BUFFER_ZONE_HIGH
        metadata["in_buffer_zone"] = in_buffer_zone
        logger.debug(f"[MemoryDB] 更新缓冲区状态: token_count={token_count}, in_buffer_zone={in_buffer_zone}")
        ...
        while i < len(conversations):
            if i + 1 < len(conversations):
                ...
                if user_msg.get("role") == "user" and assistant_msg.get("role") == "assistant":
                    if user_msg.get("daily_summarized", False) and assistant_msg.get("daily_summarized", False):
                        pair_tokens = 0
                        if "token_usage" in assistant_msg and isinstance(assistant_msg["token_usage"], dict):
                            pair_tokens = assistant_msg["token_usage"].get("total", 0)
                        
                        if total_tokens + pair_tokens > SUMMARY_TARGET_TOKENS and total_tokens > 0:
                            break
                        ...
                        i += 2
                    elif not user_msg.get("daily_summarized", False) or not assistant_msg.get("daily_summarized", False):
                        pair_tokens = 0
                        if "token_usage" in assistant_msg and isinstance(assistant_msg["token_usage"], dict):
                            pair_tokens = assistant_msg["token_usage"].get("total", 0)
    ...
    def _remove_summarized_records(self, records_to_remove):
        ...
        with self._buffer_lock:
            ...
            for record in records_to_remove:
                if record.get("role") == "assistant" and "token_usage" in record:
                    token_usage = record["token_usage"]
                    if isinstance(token_usage, dict):
                        removed_tokens += token_usage.get("total", 0)
            
            ids_to_remove = {r["msg_id"] for r in records_to_remove}
            self.context_buffer = [self.context_buffer[0]] + [
                r for r in self.context_buffer[1:] if r["msg_id"] not in ids_to_remove
            ]
            
            self.context_buffer[0]["token_count"] -= removed_tokens
            self._update_buffer_zone_status()
            self.save_context()
    ...
    def _execute_context_summary(self):
        ...
        records, total_tokens = self._select_records_for_summary()
        
        if not records:
            logger.debug("[MemoryDB] 没有记录需要总结")
            return
        
        logger.info(f"[MemoryDB] 选择了 {len(records)} 条记录进行总结，总计 {total_tokens} token")
        ...
        if summarized_records:
            logger.info(f"[MemoryDB] 处理 {len(summarized_records)} 条已总结记录")
